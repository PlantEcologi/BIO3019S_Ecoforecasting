...
You're probably wondering *"What are ecological forecasts?"* or *"Where are we going with all this?"* The focus of this section is to provide some context.

Firstly, there's a long list of existing (or in development) ecological forecasting projects on the [Ecological Forecasting Initiative's website](https://ecoforecast.org/member-forecasting-profiles/) that you can peruse. These include diverse forecasts for (*inter alia*):
...
-   water level/quality/temperature
-   disease spread/risk
-   fishing quotas
...
**What are the ecological forecasting needs in the Fynbos Biome?**

```{r fynbos, echo=FALSE, fig.cap = "", fig.width=3, fig.align = 'center', out.width="80%"}
knitr::include_graphics("img/CFR_Seasonality.gif")
```

The figure below from @Slingsby2023 outlines a set of different examples and needs for the Fynbos. I was going to try to cover all of them in this lecture, but rapidly realized it would be too long. The cool idea proposed by the paper is that many of the data streams and the informatics pipeline could be shared across multiple forecasts, reducing the overhead and maximising the versatility of the system. One could also learn a lot by comparing forecasts at different scales - e.g. the behaviour of plant populations versus the entire ecosystem.

For the rest of this section, I'm going to focus on one example of what I believe is a forecasting need and opportunity in the Fynbos Biome.  I'll be using the other examples to illustrate various principles in later lectures, but hopefully this example will provide some practical context for some of the issues we'll address in the rest of the module.
...
1.  At the *species level*, for setting guidelines for **sustainable wild harvesting** of their inflorescences
2.  At the *ecosystem level*, to help determine **acceptable fire return intervals**
...
The management guidelines are currently set by "rule of thumb"[^02-forecasting-1]:

[^02-forecasting-1]: <https://en.wikipedia.org/wiki/Rule_of_thumb>

> 1.  **Wildflower harvesting:** *"[There should be no] harvesting until at least 50% of the population had commenced flowering, a harvest of up to 50% of current season flower heads after this stage, and no harvesting at least one year prior to a prescribed burn" [@vanWilgen2016]*
