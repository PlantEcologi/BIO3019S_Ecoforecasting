...
## Where are we? {.smaller background-color="#c2c190"}
...
-   Iterative ecological forecasting represents a learning cycle that mirrors (and integrates with) the scientific method and adaptive management, helping us:
    - learn by doing
    - make our science more useful
...
-   It also presents new challenges (and opportunities) to the way we do science, requiring:
    1. informatics infrastructure to support our forecasts
    2. statistical methods that allow:
        - automatic updates of forecasts for the next time step as new data are acquired
        - quantification and propagation of uncertainty in our forecasts
...
**Before I can introduce Bayes**, there are a few basic building blocks we need to establish.

<br>

1.  How the method of ***Least Squares*** works, and its limitations, especially its inflexible, implicit data model.

. . .

2.  The concept of likelihood and the method of ***maximum likelihood estimation***
    -   this is a major component of Bayes' Theorem
    -   it's a frequentist method, but has the advantages 1 & most of 2 on previous slide
...
> Least Squares analyses don't explicitly include a data model, because minimizing the sums of squares means that ***the data model in a Least Squares analysis can only ever be a [normal (Gaussian) distribution](https://en.wikipedia.org/wiki/Normal_distribution)*** (i.e. there must be homogeneity of variance)

## Why is this a limitation? {.smaller background-color="#c2c190"}

a)  the ***data model can take many forms*** (= drawn from different [probability distributions](https://en.wikipedia.org/wiki/List_of_probability_distributions))
    -   e.g. [Binomial](https://en.wikipedia.org/wiki/Binomial_distribution) coin flips, [Poisson](https://en.wikipedia.org/wiki/Poisson_distribution) counts of individuals, [Exponential](https://en.wikipedia.org/wiki/Exponential_distribution) waiting times, etc
    -   this is where Maximum Likelihood comes into it's own
        -   in the practical, we specify two functions for the MLE analyses:
            ...
            -   the likelihood function (`fit.negexp/S.MLE`), which includes the ***data model*** (and this does not have to be a normal distribution)
...
-   It ***tells you nothing about the probability of the parameters actually being the estimates you arrived at by minimizing the sums of squares***!!!?
...
::: fragment
***Maximum Likelihood Estimation (MLE)*** = a method for estimating model parameters by applying the likelihood principle. 

  - It optimizes parameter values to maximize the likelihood that the process described by the model produced the data observed.
...
#| fig.align: center
#| out.width: 100%
#| fig.cap: The probability of a data point (dotted line) being generated under two alternative hypotheses (sets of parameter values - in this case the means and standard deviations describing the two normal distributions). <br> <br> Here H1 has a higher likelihood, because the probability density at $x$ = 1 is higher for H1 than H2 (roughly 0.25 vs 0.025 = 10 times more likely). Usually there'd be a continuum of hypotheses (parameter values) to select from, and there'd be far more data points.
...
*Probability* = area under the curve (whole curve sums to 1)

*Likelihood* = y-axis values (i.e. probability densities)

<br>

Likelihood is maximised by shifting the distribution (i.e. changing the mean and SD parameters) such that the data points have the highest likelihood. This maximizes the conditional probability of the data given those parameter values for the model.
...
[This video with ukulele music may help.](https://www.youtube.com/watch?v=pYxNSUDSFH4)
...
With MLE we assume we have the correct model and use MLE to choose parameter values that maximize the *conditional probability* of the data given those parameter values, i.e. $P(Data|Parameters)$.
...
::: {.incremental}
-   the probability of being in set $x$ ***given*** we're only considering points in set $y$ (= 3/6)
-   the probability of being in set $y$ ***given*** we're only considering points in set $x$ (= 3/7)
:::
:::
:::
...
These are just the separate probabilities of any of the full set of events being in set $x$ or being in set $y$, i.e.
...
-   if we don't have much previous knowledge, we can specify the prior to represent this by indicating our uncertainty


## The influence of the prior vs the data? {.smaller background-color="#33431e"}

```{r priorVlikelihood, echo=F, message=F, fig.cap = "Comparison of the effects on the posterior probability situations where there is (A) high uncertainty in the prior and low observation error in the data (the likelihood), versus (B) low uncertainty in the prior and high observation error. Note that the data and prior have the same means in panels A and B, but the variances differ.", fig.align = 'center', out.width="75%"}
...
## The power of the prior {.smaller background-color="#33431e"}

1.  It allows Bayesian analyses to be ***iterative***. 
