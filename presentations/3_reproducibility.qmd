...
::: incremental
-   A ***reproducible workflow*** that can be run on new data

<br>

-   ***Automation*** of the workflow, to allow it to run without human intervention
:::
...
::::: columns
::: {.column width="50%"}
```{r datalifecycle2, echo=FALSE, fig.cap = "The Data Life Cycle, adapted from https://www.dataone.org/", fig.align = 'center', out.width="100%"}
#| fig-width: 10
#| fig-height: 10
#| echo: false
...
-   Low latency
    -   No use making a forecast for the next week if we only get the latest data in a month's time...
...
::: {.column width="60%"}
-   **Automation** - code is reusable, no errors
-   A script provides a **record of your analysis**
-   **Uninterrupted workflows** - coding languages like Python or R can run many analyses in one workflow
    -   GIS, phylogenetics, statistics, visualisation, etc
    -   saves manual transfer between softwares
-   Most coding languages are **open source** 
    -   **Free!** No one has to pay to reuse your code
    -   **Transparent** - You (and others) can check the code and functions used, not just the software company
...
-   start with a "**recipe**"
    ...
    -   helps ID need for functions, split jobs among collaborators etc
